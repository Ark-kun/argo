...
var autoGeneratedPathsAndIoPathPlaceholders = `
apiVersion: argoproj.io/v1alpha1
kind: Workflow
metadata:
  generateName: artifact-path-placeholders-
spec:
  ...
  - name: head-lines
    inputs:
      ...
      artifacts:
      - name: text
#        path: /tmp/argo/input_artifacts/text/data
    outputs:
      parameters:
      - name: actual-lines-count
#        valueFrom:
#          path: /tmp/argo/output_parameters/actual-lines-count/data
      artifacts:
      - name: text
#        path: /tmp/argo/output_artifacts/text/data
    container:
      image: busybox
      command: [sh, -c, 'head -n {{inputs.parameters.lines-count}} <"{{inputs.artifacts.text.path}}" | tee "{{outputs.artifacts.text.path}}" | wc -l > "{{outputs.parameters.actual-lines-count.path}}"']
`

func TestAutoGeneratePathsAndResolveIOPathPlaceholders(t *testing.T) {
	wf := unmarshalWF(autoGeneratedPathsAndIoPathPlaceholders)
	woc := newWoc(*wf)
	woc.controller.Config.ArtifactRepository.S3 = new(S3ArtifactRepository)
	woc.operate()
	assert.Equal(t, wfv1.NodeRunning, woc.wf.Status.Phase)
	...
	assert.Equal(t, []string{"sh", "-c", "head -n 3 <\"/tmp/argo/input_artifacts/text/data\" | tee \"/tmp/argo/output_artifacts/text/data\" | wc -l > \"/tmp/argo/output_parameters/actual-lines-count/data\""}, pods.Items[0].Spec.Containers[1].Command)
}
